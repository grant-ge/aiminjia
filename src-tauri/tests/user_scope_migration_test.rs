...
fn skills_migration_copies_user_skills_but_not_builtins() {
    ...
    fs::write(root_path.join("skills/_builtins/some-builtin/plugin.toml"), "").unwrap();
    ...
    fs::write(root_path.join("skills/my-custom-skill/plugin.toml"), "custom").unwrap();
...
fn full_upgrade_flow_bootstrap_then_split_then_migrate() {
    ...
    fs::write(root_path.join("conversations/conv-abc/conv.json"), r#"{"id":"conv-abc","title":"测试"}"#).unwrap();
    fs::write(root_path.join("conversations/conv-abc/messages.jsonl"), r#"{"role":"user","content":"hello"}"#).unwrap();
    fs::write(root_path.join("index.json"), r#"{"conversations":[{"id":"conv-abc"}]}"#).unwrap();
    fs::create_dir_all(root_path.join("shared/memory")).unwrap();
    fs::write(root_path.join("shared/memory/memory.jsonl"), "[]").unwrap();
    fs::write(root_path.join("mcp_servers.json"), r#"[{"name":"test-mcp"}]"#).unwrap();
    fs::create_dir_all(root_path.join("schedules")).unwrap();
    fs::write(root_path.join("schedules/sched1.json"), r#"{"id":"sched1"}"#).unwrap();

    let global_dir = root_path.join("global");
    fs::create_dir_all(&global_dir).unwrap();

    // Step 1: bootstrap cloud_auth（相当于 lib.rs 中 AuthManager::restore 之前）
    app_lib::storage::migration_user_scope::bootstrap_cloud_auth_if_needed(
        root_path, &global_dir,
    ).unwrap();
    ...
    // Step 3: 迁移用户数据
    app_lib::storage::migration_user_scope::migrate_legacy_to_user_scope_if_needed(
        root_path, &user_dir, scope_key, &global_dir.join("state.json"),
    ).unwrap();

    // Step 4: 拆分 config
    app_lib::storage::migration_user_scope::migrate_legacy_config_if_needed(
        root_path, &user_dir, &global_dir,
    ).unwrap();
    ...
    assert!(user_dir.join("conversations/conv-abc/messages.jsonl").exists());
    assert!(user_dir.join("index.json").exists());
    assert!(user_dir.join("shared/memory/memory.jsonl").exists());
    assert!(user_dir.join("mcp_servers.json").exists());
    ...
    let state: serde_json::Value = serde_json::from_str(
        &fs::read_to_string(global_dir.join("state.json")).unwrap(),
    ).unwrap();
    assert_eq!(state["migrations"]["legacyRootClaim"]["claimedBy"], scope_key);

    // ========== 切换账号后 B 看不到 A 的数据 ==========
    let scope_b = "t_3__u_4";
    let user_dir_b = root_path.join("users").join(scope_b);
    app_lib::storage::migration_user_scope::migrate_legacy_to_user_scope_if_needed(
        root_path, &user_dir_b, scope_b, &global_dir.join("state.json"),
    ).unwrap();
...
fn file_manager_workspace_updates_on_scope_switch() {
    ...
    assert_eq!(fm.workspace_path(), std::path::PathBuf::from("/tmp/old_workspace"));

    // 模拟登录后更新
    fm.update_workspace_path("/tmp/new_workspace");
    assert_eq!(fm.workspace_path(), std::path::PathBuf::from("/tmp/new_workspace"));

    // 模拟登出后重置
    fm.update_workspace_path("/tmp/default_root");
    assert_eq!(fm.workspace_path(), std::path::PathBuf::from("/tmp/default_root"));
}
...
fn config_split_idempotent_when_global_config_exists() {
    ...
    assert_eq!(fs::read_to_string(global_dir.join("config.json")).unwrap(), first_content);
