...
try:
    ...
    if '_dfs' in dir() and _dfs:
        ...
        if 'balance_sheet' in detected:
            df_bs = detected['balance_sheet'][1]
    if df_pl is None and '_df' in dir() and _df is not None:
        df_pl = _df

    if df_pl is not None:
        result = _step2_financial_ratios(df_pl, df_bs)
    else:
        result = {'error': '未找到利润表数据，无法计算财务比率'}
except Exception as e:
    result = {'error': str(e)}

_cache_result('finance_step2', result)
