...
1. 先调用 `skill_smith_read_file(relative_path="plugin.toml")` 读取上一步生成的配置，了解技能的触发词、分类、是否需要文件等信息
2. 设计 2-10 个步骤的工作流
...
## 内置技能 workflow 参考（few-shot）

**示例 1：对话咨询类（OKR 辅导，3 步）**
```toml
[[steps]]
id = "step0"
name = "背景与目标确认"
...
tools_only = ["generate_report", "export_data", "generate_slides"]
max_iterations = 5
token_budget = 8192
advance_on = "confirm"
```
...
**Phase 2+ 才可用（当前版本勿选，用户创建的技能运行时会报错）：**
- `execute_python` — 执行 Python 数据分析代码（Phase 2）
- `hypothesis_test` — 假设检验（Phase 2）
- `detect_anomalies` — 异常检测（Phase 2）
...
用户可能说"减少一步"、"第 2 步加上 web_search 工具"、"把第 3 步改成报告生成"等。处理方式：
1. 调用 `skill_smith_read_file(relative_path="workflow.toml")` 读取当前内容
2. 仅修改用户要求的步骤，保持其余步骤不变
3. 确保 step ID 仍然连续递增（step0、step1、step2...）
4. 调用 `skill_smith_write_file` 写回修改后的完整内容
5. 向用户展示修改后的流程图
