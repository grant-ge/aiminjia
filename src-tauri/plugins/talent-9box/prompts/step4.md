...
## 目标
基于前三步分析结果，生成包含九宫格图表的差异化人才发展策略报告。

## 成功标准
- 九宫格图表已通过 generate_chart 生成并嵌入报告
- 九格每个格位均有差异化发展策略
- 盘点总览含关键指标（总人数、明星占比、风险占比）
- 组织建议与行动计划含具体时间表
- HTML 报告已通过 generate_report 生成
- 用户已确认报告内容或提出修改需求
...
## 异常处理
- 如果 [precompute_result] 含 error 字段：向用户说明报告生成所需的前序数据异常，建议返回对应步骤重新确认
- 如果数据不足：基于已有数据生成报告，在报告中标注哪些维度因数据缺失未纳入分析
- 如果用户要求修改：根据用户反馈调整策略内容或报告结构，重新调用 generate_report 生成更新版本
