...
result = {}
try:
    _types = _KNOWLEDGE.get('proposal_types', {}) if '_KNOWLEDGE' in dir() else {}
    _structures = _KNOWLEDGE.get('structures', {}) if '_KNOWLEDGE' in dir() else {}
    _rules = _KNOWLEDGE.get('writing_rules', {}) if '_KNOWLEDGE' in dir() else {}

    result = {
        'proposal_types': _types,
        'structures': _structures,
        'writing_rules': _rules,
        'note': '基于知识库推荐方案类型和结构，请根据用户需求个性化调整'
    }
except Exception as e:
    result = {'error': str(e)}

_cache_result('biz_proposal_step0', result)
