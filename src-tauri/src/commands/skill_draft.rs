//! Tauri commands for skill package import/export.
//!
//! Frontend uses these to:
//! - Import `.zip` packages from disk (drag-drop / file association / button)
//! - Export an installed skill as `.zip` (SkillCard "export" menu)
//!
//! Both commands operate on disk only and do not touch any draft store —
//! the historical draft system has been removed. The file name `skill_draft.rs`
//! is preserved for git history continuity; rename later if desired.
