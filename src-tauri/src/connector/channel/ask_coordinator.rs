        ...
        log::info!(
            ...
            match &judgement {
                JudgeResult::PermissionAnswered { allow, .. } =>
                    if *allow { "permission_allow" } else { "permission_deny" },
    ...
    async fn judge_user_question(
        ...
        match parsed {
            Ok(v) if v.verdict == "answered" => match v.answers {
                ...
                None => JudgeResult::Ambiguous { reason: "answered without answers".into() },
            },
            Ok(v) if v.verdict == "abandoned" => JudgeResult::Abandoned {
                reason: v.reason.unwrap_or_else(|| "user changed topic".into()),
            },
...
mod tests {
    ...
    async fn deadline_denies_permission_and_clears_slot() {
        ...
        let judge = Arc::new(ScriptedJudge {
            result: StdMutex::new(JudgeResult::Ambiguous { reason: "unused".into() }),
        });
    ...
    async fn answered_permission_is_consumed() {
        let coordinator = make_coordinator(Arc::new(ScriptedJudge {
            result: StdMutex::new(JudgeResult::PermissionAnswered { allow: true, reason: "user allowed".into() }),
        }));
    ...
    async fn abandoned_reply_is_rerouted() {
        let coordinator = make_coordinator(Arc::new(ScriptedJudge {
            result: StdMutex::new(JudgeResult::Abandoned { reason: "new topic".into() }),
        }));
        coordinator.pending.lock().await.insert("sess-im".into(), PendingAsk {
            session_id: SessionId::new("sess-im"),
            run_id: RunId::new("run-1"),
