...
use super::types::{AttachmentKind, ChannelAttachmentSpec, ChannelConnectionState, ChannelMessage, ConversationType};
...
pub async fn send_session_webhook_text(session_webhook: String, text: String) {
        ...
        .await
    {
        ...
        Err(e) => {
            log::warn!("[dingtalk-stream] sessionWebhook auto-reply failed: {:#}", e);
        }
    }
}
...
mod tests {
    ...
    fn dingtalk_doc_card_triggers_auto_reply() {
        ...
        match client.parse_im_message(data) {
            ...
            other => panic!("expected AutoReply, got {:?}", match other {
