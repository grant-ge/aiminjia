...
pub struct ChatMessage {
    ...
    pub is_error: bool,
    /// Thinking/reasoning content from the model. Must be passed back on
    /// subsequent turns for providers that require signed thinking context.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub thinking: Option<String>,
    /// Full thinking blocks with upstream signatures. These are opaque and
    /// must be echoed back verbatim so the upstream can validate them.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub thinking_blocks: Option<Vec<serde_json::Value>>,
    /// Current-turn image sidecar for providers that require structured image
    /// blocks. This is never persisted.
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct LlmRequest {
    ...
    pub anthropic_multimodal_turn: Option<AnthropicMultimodalTurn>,
    /// Per-block cache_control hints for gateway providers that support prompt
    /// caching. Other providers ignore this and fall back to reading the
    /// flattened system message from `messages`.
    pub system_segments: Option<Vec<SystemPromptSegment>>,
    /// Conversation id, when known. Sent to AIjia Gateway V2 for traceability
    /// and conversation-level routing hints.
