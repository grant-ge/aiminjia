...
pub fn validate_team_name(raw: &str) -> Result<(), TeamNameError> {
    if raw.is_empty() {
        return Err(TeamNameError::TooShort);
    }
    if raw.len() > 64 {
        return Err(TeamNameError::TooLong { len: raw.len() });
    }
    if !raw
        .chars()
...
impl<'a> TeamPaths<'a> {
    ...
    pub fn config_json(&self) -> PathBuf {
        self.team_root()
            .expect("config_json requires team-bound TeamPaths")
            .join("config.json")
    }

    pub fn team_chat_jsonl(&self) -> PathBuf {
        self.team_root()
            .expect("team_chat_jsonl requires team-bound TeamPaths")
            .join("team-chat.jsonl")
    }
    ...
    pub fn teammates_dir(&self) -> PathBuf {
        self.team_root()
            .expect("teammates_dir requires team-bound TeamPaths")
            .join("teammates")
    }
...
mod tests {
    ...
    #[test]
    fn validate_accepts_simple_ascii() {
    ...
    fn validate_rejects_empty() {
        assert_eq!(validate_team_name(""), Err(TeamNameError::TooShort));
    }
    #[test]
    fn validate_rejects_too_long() {
        let s = "a".repeat(65);
        assert_eq!(
            validate_team_name(&s),
            Err(TeamNameError::TooLong { len: 65 })
        );
    }
    #[test]
    ...
    #[test]
    fn validate_rejects_chinese() {
        assert_eq!(
            validate_team_name("市场调研"),
            Err(TeamNameError::InvalidChars)
        );
    }
    #[test]
    ...
    fn validate_rejects_space() {
        assert_eq!(
            validate_team_name("research team"),
            Err(TeamNameError::InvalidChars)
        );
    }
    #[test]
    ...
    fn validate_rejects_windows_reserved() {
        for name in &[
            "CON", "con", "Con", "PRN", "prn", "AUX", "NUL", "COM1", "com9", "LPT5",
        ] {
            assert!(matches!(
                validate_team_name(name),
                Err(TeamNameError::WindowsReserved(_))
            ));
        }
    }
    #[test]
    fn validate_accepts_reserved_prefix() {
        assert!(validate_team_name("CONFIG").is_ok());
        assert!(validate_team_name("PRINTER").is_ok());
        assert!(validate_team_name("COM10").is_ok());
    }
    #[test]
    fn validate_rejects_all_dashes() {
        assert_eq!(
            validate_team_name("---"),
            Err(TeamNameError::DegenerateName("---".to_string()))
        );
        assert_eq!(
            validate_team_name("-"),
            Err(TeamNameError::DegenerateName("-".to_string()))
    ...
    #[test]
    fn team_root_for_conv_returns_none() {
        assert_eq!(TeamPaths::for_conv(&conv()).team_root(), None);
    }
    #[test]
    fn team_root_for_team() {
        let dir = conv();
        assert_eq!(
            TeamPaths::for_team(&dir, "alpha").team_root(),
            Some(dir.join("teams").join("alpha"))
        );
    }
    #[test]
    fn config_json_for_team() {
        let dir = conv();
        assert_eq!(
            TeamPaths::for_team(&dir, "alpha").config_json(),
            dir.join("teams/alpha/config.json")
        );
    }
    #[test]
    #[should_panic]
    fn config_json_for_conv_panics() {
        let _ = TeamPaths::for_conv(&conv()).config_json();
    }
    #[test]
    fn team_chat_for_team() {
        let dir = conv();
        assert_eq!(
            TeamPaths::for_team(&dir, "alpha").team_chat_jsonl(),
            dir.join("teams/alpha/team-chat.jsonl")
        );
    }
    #[test]
    fn tasks_dir_for_conv() {
        let dir = conv();
        assert_eq!(TeamPaths::for_conv(&dir).tasks_dir(), dir.join("tasks"));
    }
    #[test]
    fn tasks_dir_for_team() {
        let dir = conv();
        assert_eq!(
            TeamPaths::for_team(&dir, "alpha").tasks_dir(),
            dir.join("teams/alpha/tasks")
        );
    }
    #[test]
    fn teammates_dir_for_team() {
        let dir = conv();
        assert_eq!(
            TeamPaths::for_team(&dir, "alpha").teammates_dir(),
            dir.join("teams/alpha/teammates")
        );
    }
    #[test]
    fn teammate_transcript() {
        let dir = conv();
        assert_eq!(
            TeamPaths::for_team(&dir, "alpha").teammate_transcript("agent-42"),
            dir.join("teams/alpha/teammates/agent-42.jsonl")
        );
    }
    #[test]
    fn teammate_meta() {
        let dir = conv();
        assert_eq!(
            TeamPaths::for_team(&dir, "alpha").teammate_meta("agent-42"),
            dir.join("teams/alpha/teammates/agent-42.meta.json")
        );
    }
}
