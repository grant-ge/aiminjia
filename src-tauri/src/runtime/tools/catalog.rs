...
fn build_default_catalog() -> ToolCatalog {
    ...
    c.insert(CatalogEntry::new(
        ToolDefinition::new(
            ...
            \n\n交付规则：需要创建或更新文本产物时，优先使用 Write/Edit；若需要计算、解析或转换后生成结果，优先用一次 shell 调用完成“读取输入 -> 计算/转换 -> 写入用户指定目标路径 -> 打印简短校验摘要”。命令后要用独立读取、列举或测试确认目标路径存在、非空且格式合理。若任务包含调度、资源分配、优化或硬约束，校验必须读取最终写出的文件/payload，并逐项检查输出字段里的每个 attendee/assignee/resource/equipment；optional/candidate/backup 实体只要被写入最终输出，就按已安排/已分配处理，不能违反硬约束。字段级校验不能只做 `ls`、`grep -c`、事件数量、原始 UID 数量或 JSON 可解析检查；应写出会失败退出的断言脚本。iCalendar/ICS 要解析每个 `BEGIN:VEVENT` 的 `UID`、`DTSTART`、`DTEND`、`ATTENDEE` 并和 unavailability/容量/冲突规则比对；JSON/CSV 要解析 schema 和关键字段；Markdown 精确结构要检查必需标题且避免额外章节。断言失败时先修目标文件，再总结。\
    ...
    c.insert(CatalogEntry::new(
        ToolDefinition::new(
            ...
            \n\n交付规则：需要创建或更新文本产物时，优先使用 Write/Edit；若需要计算、解析或转换后生成结果，优先用一次 PowerShell 调用完成“读取输入 -> 计算/转换 -> 写入用户指定目标路径 -> 打印简短校验摘要”。命令后要用 Get-Item/Get-Content/Test-Path 或项目校验命令确认目标路径存在、非空且格式合理。若任务包含调度、资源分配、优化或硬约束，校验必须读取最终写出的文件/payload，并逐项检查输出字段里的每个 attendee/assignee/resource/equipment；optional/candidate/backup 实体只要被写入最终输出，就按已安排/已分配处理，不能违反硬约束。字段级校验不能只做 Test-Path、目录列举、事件数量、原始 UID 数量或 JSON 可解析检查；应写出会失败退出的断言脚本。iCalendar/ICS 要解析每个 `BEGIN:VEVENT` 的 `UID`、`DTSTART`、`DTEND`、`ATTENDEE` 并和 unavailability/容量/冲突规则比对；JSON/CSV 要解析 schema 和关键字段；Markdown 精确结构要检查必需标题且避免额外章节。断言失败时先修目标文件，再总结。\
