...
impl ToolPlugin for MyTool {
    ...
    async fn execute(&self, ctx: &PluginContext, input: Value) -> Result<ToolOutput, ToolError> {
        // ctx.storage / ctx.file_manager / ctx.workspace_path / ctx.conversation_id
...
[model]
preference = "deep_reasoning"  # deep_reasoning | cost_efficient | code_generation
...
tools_exclude = ["tool3"]            # 可选
max_iterations = 10                  # 可选
requires_confirmation = true         # 可选，默认 true
```

步骤按数组顺序执行。`tools_only` > `tools_exclude` > 全部可用。
...
`tauri.conf.json` 已配置 `"plugins": "plugins"`，构建时自动打包到应用资源目录，启动时自动扫描加载。
