...
| **Skill command forwarding** — 接收前端传入的 skill command，并交给统一 runtime chat loop 处理 | `send_message` adapter path | current | `skill_command`, `RuntimeChatTurnDriver` | runtime 负责后续状态推进 | Command |
