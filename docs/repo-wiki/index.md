...
- 40 个 guided tour steps
- 139 个 LLM-enhanced 节点
- 27 个代码/维护架构评审概念节点
- 9 份当前源码/测试/skill 来源 enhancement JSON

## Architecture Layers

| Layer | 说明 | 规模 |
|---|---:|---:|
| 文档与工程约束 | AGENTS、CLAUDE、architecture blueprint、decisions、release 和 test-intents 的权威入口 | 96 |
...
| 代码架构评审 | 当前源码、测试和 repo-local skill 来源的增强材料生成的 architecture review 概念节点 | 27 |
...
5. Tauri Transport 进入 Runtime：chat command、runtime host、`SessionRuntime`、`TauriEventAdapter`。
...
10. 测试与兼容护栏：CI、`review_*.rs`、`send_message_runtime_path_test`、test-intents。
11. LLM 网关与流式协议：gateway、router、provider、streaming、event bus、event adapter。
12. Workspace 与文件安全边界：CurrentUserStorage、WorkspaceManager、FileManager、file_store、path_auth。
...
15. 代码增强 tour：新增 23 个以代码/测试为事实源的 guided tour steps，覆盖前端 chat、employee/settings/file preview、skill/pending、LLM gateway、managed runtime、MCP、runtime permission 和 storage/path_auth。
16. UserWiki skill tour：新增 3 个以 repo-local skill/script 为事实源的 guided tour steps，覆盖 userwiki 问答入口、wiki-maintainer 维护入口和校验脚本。
