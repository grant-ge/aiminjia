...
| LLM gateway / provider / streaming | strong | `.understand-anything/enhancements/llm-gateway-provider-streaming.json`, `.understand-anything/enhancements/llm-visible-reply-language-anchor.json`, `docs/repo-wiki/runtime-map.md` | AIjia v2 visible reply language anchor 与中文回复回归 intents 已基于 `origin/main@c4bcc8b7` 补入图谱；后续 provider request builder 或语言约束策略变化时再补 |
...
| Managed runtime supply chain | strong | `.understand-anything/enhancements/managed-runtime-supply-chain.json`, `docs/repo-wiki/runtime-map.md`, `src-tauri/tests/runtime_dependencies_manager_test.rs`, `src-tauri/tests/runtime_dependencies_no_legacy_resource_test.rs`, `src-tauri/tests/managed_runtime_process_env_test.rs` | 安装包随带 Node/Python/uv fallback、`resources/dws` 和旧 bundled graph 节点已清理；后续 runtime manifest、下载、诊断或 env 注入策略变化时再补 |
...
| Skill enablement / marketplace sync / runtime catalog | queued | `docs/repo-wiki/writeback-queue.md` | WB-2026-06-10-001：实现完成并验证后补 skill enablement、marketplace sync、runtime catalog、AEIT CLI coverage |
...
| Agent foreground auto-background | strong | `.understand-anything/enhancements/runtime-agent-foreground-auto-background.json`, `docs/repo-wiki/runtime-map.md`, `src-tauri/src/runtime/tools/builtin/spawn_subagent.rs`, `src-tauri/src/llm/tool_executor/spawn_subagent.rs`, `src-tauri/src/runtime/agent/async_task_store.rs`, `src-tauri/src/runtime/tools/builtin/task_output.rs`, `src-tauri/src/runtime/tools/builtin/task_stop.rs`, `src-tauri/tests/spawn_subagent_auto_background_test.rs`, `src-tauri/tests/spawn_subagent_async_test.rs` | 当前覆盖最终输出、停止和通知；若未来要求逐 token 实时 transcript 连续，需要补更细粒度 streaming 回归 |
| Shell auto-background / background task IO | strong | `.understand-anything/enhancements/runtime-shell-auto-background.json`, `docs/repo-wiki/runtime-map.md`, `src-tauri/src/runtime/tools/builtin/bash.rs`, `src-tauri/src/runtime/tools/builtin/powershell.rs`, `src-tauri/src/runtime/tools/builtin/shell_task.rs`, `src-tauri/src/runtime/tools/builtin/task_output.rs`, `src-tauri/src/runtime/tools/builtin/task_stop.rs` | PowerShell 自动后台化行为级回归仍弱于 Bash；Agent 已有独立 LocalAgent 自动后台化链路，注意不要混同 LocalBash handoff |
