...
| Agent foreground auto-background target gap | partial | `.understand-anything/enhancements/runtime-agent-foreground-auto-background-gap.json`, `docs/repo-wiki/runtime-map.md`, `src-tauri/src/runtime/tools/builtin/spawn_subagent.rs`, `src-tauri/src/llm/tool_executor/spawn_subagent.rs`, `src-tauri/src/runtime/agent/agent_runtime.rs`, `src-tauri/src/runtime/agent/worker_runtime.rs` | 当前只确认缺口和目标链路；实现前台 Agent promotion、补测试后再升级为 strong |
| Shell auto-background / background task IO | strong | `.understand-anything/enhancements/runtime-shell-auto-background.json`, `docs/repo-wiki/runtime-map.md`, `src-tauri/src/runtime/tools/builtin/bash.rs`, `src-tauri/src/runtime/tools/builtin/powershell.rs`, `src-tauri/src/runtime/tools/builtin/shell_task.rs`, `src-tauri/src/runtime/tools/builtin/task_output.rs`, `src-tauri/src/runtime/tools/builtin/task_stop.rs` | PowerShell 自动后台化行为级回归仍弱于 Bash；Agent 前台自动转后台仍是当前源码缺口 |
