...
name: browser
description: >
  浏览器自动化能力。用 playwright-cli 打开网页、点击、填表、抓取数据、截图、保存登录态。
  适用：普通网页、没有专用技能覆盖的 Web 页面、或 find-skills 未找到专用技能后的兜底浏览器操作。
  不适用：用 HTTP API / SQL 就能拿数据（走 Bash + curl）；IE 锁定的政务/银行专用浏览器（走 RPA）。
  不适用：用户点名钉钉、睿人事、CRM、OA、招聘后台等具名企业产品或业务系统，但本地没有对应专用技能时；这种场景先用 find-skills 查市场。
when_to_use: >
  用户说"去某个网页 / 某个网站"做"看 / 查 / 登录 / 操作 / 抓 / 截图 / 翻页 / 提取 / 导出"等动作，
  或要求操作普通 Web UI、自动登录某站点、提取页面表格/列表数据、脚本化浏览器操作，触发本 skill。
  如果用户说的是具名企业产品或业务系统，且 catalog 里没有该产品的专用技能，不要直接触发本 skill；先调用 find-skills 发现专用技能。
allowed-tools:
  - Bash
  - Read
  - Write
