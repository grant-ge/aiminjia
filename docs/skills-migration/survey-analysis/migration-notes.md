# survey-analysis migration notes

## 来源

- 旧目录：`src-tauri/plugins/survey-analysis`（本次复核通过 `git ls-tree` / `git show HEAD:...` 读取，未恢复已删除的旧目录）
- 已复核：`plugin.toml`、`workflow.toml`、`prompts/*.md`、`scripts/knowledge/*`
- 新入口：`docs/skills-migration/survey-analysis/SKILL.md`
...
- 知识库：`references/knowledge/benchmarks.json`, `references/knowledge/action_playbooks.json`
