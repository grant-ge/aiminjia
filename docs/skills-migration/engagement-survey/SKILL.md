...
可使用 `execute_python` 做计算，使用 `export_data` 导出结果表。展示给用户时只给业务结论和必要口径，不展示代码。询问用户整体指标是否合理，是否需要调整字段映射或评分口径。
...
必要时使用 `execute_python` 计算和 `export_data` 导出分组对比表。询问用户是否认可分组结果，是否要深入某个群体。
...
相关性不是因果。输出时要区分“数据相关”“可能原因”和“需要业务访谈验证的假设”。必要时使用 `execute_python` 和 `export_data`，并将分析口径保存为说明。
...
用户确认后可使用 `generate_report` 生成报告，建议包含调研概况、样本分布、eNPS、整体敬业度、维度排名、分组差异、根因诊断、行动计划和下次调研建议。如用户需要汇报材料，再使用 `generate_slides`，每页 4-6 条要点，突出数字和行动。
