# policy-compliance-audit migration notes

## 来源

- 旧目录：`src-tauri/plugins/policy-compliance-audit`（本次复核通过 `git ls-tree` / `git show HEAD:...` 读取，未恢复已删除的旧目录）
- 已复核：`plugin.toml`、`workflow.toml`、`prompts/*.md`、`scripts/knowledge/*`
- 新入口：`docs/skills-migration/policy-compliance-audit/SKILL.md`
...
- 知识库：`references/knowledge/labor_law_index.json`, `references/knowledge/invalid_clause_patterns.json`, `references/knowledge/procedure_rules.json`
