...
agent_loop(AgentContext, messages, StepConfig)
    ...
    └─ Loop (max_iterations):
         ├─ 构建 dynamic_ctx（每次迭代更新：file_context + notes + AnalysisContext + plan）
...
1. **Schema 层**：始终向 LLM 暴露全部 11 个工具 Schema（保持 KV cache 前缀稳定）
...
日常模式 runtime guard 排除 5 个分析专用工具（hypothesis_test, detect_anomalies, save_analysis_note, update_progress, update_plan）。
...
动态上下文:    [file_context] + [analysis_notes] + [AnalysisContext] + [step_plan]  ← 独立 context_message
```
