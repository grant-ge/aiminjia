...
检测到 Skill 激活后，不立即进入分析模式，而是：
1. 将 Skill ID 存为 `pending_skill`（enterprise memory）
2. 发送确认消息：「检测到您可能想要进行「XX」。如果确认开始，请回复「确认」或「开始」」
3. 等待用户下一条消息：
   - 确认词 → 正式激活 Skill，进入 `confirming` 模式
   - 取消词 → 清除 pending，继续 daily 模式
   - 其他内容 → 清除 pending，作为普通 daily 消息处理
...
conversation.mode
    │
    ├─ "daily" ───── pending_skill 检查
    │                 ├─ 有 pending + 用户确认 → 激活 Skill → confirming
    │                 ├─ 有 pending + 用户取消 → 留在 daily
    │                 ├─ 有 pending + 其他消息 → 清除 pending → DailyChat
    │                 └─ 无 pending → detect_activation()
    │                      ├─ 有 Skill 激活 → 存 pending + 发确认消息 → 等待
    │                      └─ 无 → DailyChat
    │
...
| 两阶段确认（pending skill） | `commands/chat.rs:send_message`（daily 分支） |
