...
| 联网搜索 | SearXNG（免费优先）+ Tavily（付费增强/降级） | `search/tavily.rs`, `search/searxng.rs` |
...
**auto_capture_step_context** (`chat.rs`): 在消息历史清除前，自动提取当前步骤的关键输出并保存为 `note:{conv_id}:step{N}_auto_context`（最大 4000 字符），作为 LLM 主动调用 `save_analysis_note` 的安全兜底。
...
- 最近完成步骤的 `auto_context`：最多 4000 字符
- 更早步骤的 `auto_context`：截断至 2000 字符
- 估算：~2K(base prompt) + ~2K/步 ≈ 12K tokens (5步) / 22K tokens (10步)，在 64K DeepSeek / 200K Claude 上下文内
