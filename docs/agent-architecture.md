...
| 报告生成 | HTML 富内容渲染 | `tool_executor.rs` |
| 联网搜索 | Tavily + SearXNG 降级 | `search/tavily.rs`, `search/searxng.rs` |
...
┌────────────────────────▼────────────────────────────────┐
│                   commands/chat.rs                        │
│   detect_analysis_mode → orchestrator::next_action       │
│   → agent_loop(StepConfig) → finish_agent                │
├──────────────────────────────────────────────────────────┤
│                   llm/orchestrator.rs                     │
│   模式状态机 · AnalysisAction 路由 · 确认检测            │
├──────────┬───────────┬───────────┬───────────────────────┤
│ gateway  │ masking   │ router    │ prompts               │
│ 流式请求 │ PII 脱敏  │ 模型路由  │ 系统提示词            │
├──────────┴───────────┴───────────┴───────────────────────┤
│                  llm/tool_executor.rs                     │
│   10 个 Tool Handler → Python / Search / Report / ...    │
├──────────────────────────────────────────────────────────┤
│                    storage/file_store/                    │
...
`orchestrator::detect_analysis_mode()` 通过两种方式检测：
...
**步骤切换时（`StartAnalysis` / `AdvanceStep`）**，消息列表从零构建：

```
旧步骤 (Step N)                    新步骤 (Step N+1)
─────────────────                  ─────────────────
System Prompt (Step N)             System Prompt (Step N+1)  ← 新的
User Message 1                     [前序分析记录]            ← 从 memory.jsonl 注入
Tool Call: analyze_file            User: "确认，继续"        ← 原始用户消息
Tool Result: {...}
Assistant: "数据清洗完成..."        * 前序步骤的 Tool Call
User: "确认"                          历史全部丢弃
```

### 4.3 记忆传递机制

跨步骤的关键结论通过 `save_analysis_note` → `memory.jsonl` → 系统提示词注入传递：
...
Step 1 (数据清洗)                              memory.jsonl
    ...
    │    content="有效人数 180..."              │   固浮比 7:3, 缺失率 2%"
    │  )                                       │
    │                                          │
Step 2 (岗位归一化)                             │
    │                                          │
...
### 5.2 AnalysisAction 路由枚举

`orchestrator::next_action()` 基于 `conversation.mode` 返回以下动作：

```rust
enum AnalysisAction {
    DailyChat,                    // 非分析模式 → 日常咨询
    StartAnalysis(StepConfig),    // confirming 模式 + 用户回复 → 进入 Step 1
    AdvanceStep(StepConfig),      // 用户确认当前步 → 推进到下一步
    RerunStep(StepConfig),        // 用户给反馈 → 重跑当前步
    ResumeStep(StepConfig),       // 崩溃恢复 → 恢复暂停的步骤
    FinishAnalysis,               // Step 5 确认 → finalize_analysis()
    AbortAnalysis,                // 用户取消 → 回到 daily
}
```
...
conversation.mode
    ...
    ├─ "daily" ────────────────── → DailyChat
    │
    ├─ "confirming" ────────────── 用户消息 → StartAnalysis(Step 1)
    │                               (mode 设为 "analyzing")
    │
    └─ "analyzing" ─┬─ is_abort() ──────────── → AbortAnalysis
                    │
                    └─ route_analysis_step()
                         │
                         ├─ status=InProgress → ResumeStep (崩溃恢复)
                         ├─ status=Completed
                         │    ├─ is_confirmation()
                         │    │    ├─ 当前步 < 5 → AdvanceStep(下一步)
                         │    │    └─ 当前步 = 5 → FinishAnalysis
                         │    └─ 非确认(反馈) → RerunStep(当前步)
                         └─ status=Paused → ResumeStep
```
...
| 4 | `generate_report` | 生成 HTML/Markdown 报告 | `title`*, `sections`*[], `format` |
...
`tool_executor::execute_tool()` 是中央分发器，根据 `tool_call.name` 路由到对应 handler：

```rust
match tool_call.name.as_str() {
    "web_search"       => handle_web_search(args, ctx),
    "execute_python"   => handle_execute_python(args, ctx),
    "analyze_file"     => handle_analyze_file(args, ctx),
    "generate_report"  => handle_generate_report(args, ctx),
    "generate_chart"   => handle_generate_chart(args, ctx),
    "hypothesis_test"  => handle_hypothesis_test(args, ctx),
    "detect_anomalies" => handle_detect_anomalies(args, ctx),
    "save_analysis_note" => handle_save_analysis_note(args, ctx),
    "export_data"      => handle_export_data(args, ctx),
    "update_progress"  => handle_update_progress(args, ctx),
    _                  => Err("Unknown tool"),
}
```
...
| 函数 | 功能 |
|------|------|
| `_smart_read_csv(path, **kwargs)` | 编码自动探测（UTF-8 → GBK → latin-1） |
| `_smart_read_data(path)` | CSV/Excel 自动分发 + 编码探测 |
...
三重保障：
1. Python 进程环境变量 `PYTHONIOENCODING=utf-8`
2. Preamble 中 `sys.stdout.reconfigure(encoding='utf-8')`
3. `_smart_read_csv` / `_smart_read_data` 编码自动探测（UTF-8 → GBK → latin-1）
...
| 分析意图检测 | `llm/orchestrator.rs:detect_analysis_mode` |
| 路由下一步动作 | `llm/orchestrator.rs:next_action` |
...
| 工具定义 | `llm/tools.rs:build_tool_definitions` |
| 步骤工具过滤 | `llm/tools.rs:get_tools_for_step` |
| 工具执行分发 | `llm/tool_executor.rs:execute_tool` |
| Python 代码执行 | `python/runner.rs:execute` |
| 沙箱验证 | `python/sandbox.rs:validate_code` |
| Preamble 生成 | `python/sandbox.rs:preamble` |
| HTML 报告生成 | `llm/tool_executor.rs:build_html_report` |
| 崩溃恢复扫描 | `lib.rs`（启动时） |
