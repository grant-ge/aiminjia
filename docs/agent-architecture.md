...
| Python 数据分析 | 双路径执行：分析模式持久 REPL 会话 / 日常模式一次性子进程 | `session.rs`, `runner.rs`, `sandbox.rs`, `analysis_utils.rs` |
| PII 脱敏 | 全链路 mask/unmask | `masking.rs` |
| 文件解析 | Python 解析器（CSV/Excel/JSON/Parquet/PDF/Word/PPT/HTML/Text） | `python/parser.rs` |
| 报告生成 | HTML/PDF/DOCX 富内容渲染（PDF/DOCX 从 HTML 自动转换） | `tool_executor/report.rs` |
| 联网搜索 | SearXNG（免费优先）+ Bocha/Tavily（付费增强/降级） | `search/searxng.rs`, `search/bocha.rs`, `search/tavily.rs` |
...
```
     ...
     │        （返回日常模式）                │
     │    · 清除所有分析 notes                │
     │    · 记录 analysis_completed_at        │
     │    · 60 秒内不再触发 detect_activation │
     └──────────────────────────────────────┘

     * 任何阶段用户说"取消/abort" → AbortAnalysis
       → clear_analysis_notes() → daily
```
...
检测到 Skill 激活后：
- **有文件（`has_files=true`）**→ 直接进入 `confirming` 模式，开始 Step 0（分析方向确认）
- **无文件（`has_files=false`）**→ 留在 `daily` 模式，LLM 自然回复（可引导用户上传文件）

**分析冷却期**：分析完成/中止后 60 秒内，`detect_activation()` 不会运行，避免分析完成后用户消息误触发新一轮分析。

### 2.4 分析中日常问题路由

当 conversation mode 为 `confirming` 或 `analyzing` 时，如果用户消息被 `is_daily_question()` 判定为日常 HR 问题（如"社保怎么算"、"年假政策"），则：
- 将该消息路由到 daily 模式 agent_loop（step_config = None）
- **不改变** conversation mode（仍保持 confirming/analyzing）
- 用户下次发送确认/反馈消息时，分析流程继续

判定规则：
- 消息长度 ≤ 20 字符 → 不是日常问题（可能是确认词）
- 匹配确认/取消关键词 → 不是日常问题
- 匹配日常问题模式（`请问`/`什么是`/`社保`/`政策`/`年假`等）且不含分析反馈关键词 → 是日常问题

### 2.5 mode 与 analysis.json 的职责分离
...
| 步骤 | 目标 | 工具子集 | 最大迭代 | 输出物 |
|------|------|----------|----------|--------|
| Step 0 | 方向确认 | `load_file`, `save_analysis_note` | 5 | 文件摘要 + 方向偏好 |
| Step 1 | 数据清洗 | `load_file`, `execute_python`, `save_analysis_note`, `update_progress` | 15 | 字段映射、排除清单、薪酬结构、数据质量 |
...
**7 步执行流程**：
1. 调用 `load_file(file_id)` 加载数据（如未加载），数据自动可用为 `_df`
2. Python 查看数据概览（shape + 前 5 行 + 列名）
3. 字段语义映射（基本信息 / 薪酬字段 / 汇总字段 / 辅助字段）
4. 排除无效记录（当月入职、已离职、非全职、试用期 ≤3 月、基本工资=0）
5. 导出排除人员 Excel + 聊天预览前 15 行
6. 薪酬结构分析：固定（基本工资 + 津贴 + 补贴）vs 浮动（绩效 + 提成 + 加班 + 奖金）+ 数据质量评估
7. `save_analysis_note(key="step1_summary")` 保存结论
...
Tool Call: load_file                   → 非流式 LLM 调用提取结构化       (checkpoint 优先 > summary > auto)
...
- 估算：~2K(base prompt) + ~3K/步 ≈ 20K tokens (6步) / 32K tokens (10步)，在 64K DeepSeek / 200K Claude 上下文内
...
日常模式使用滑动窗口（`MAX_HISTORY_MESSAGES=30`），但长对话中早期上下文会被硬截断丢失。自动压缩机制在 LLM 调用前检测并压缩超长历史：

```
agent_loop() 日常模式
    │
    ├─ 过滤 tool call/result 消息
    │
...
**设计要点**：
- 仅在日常模式触发（分析模式使用三层检查点机制，不需要压缩）
- 使用 `MaskingLevel::Relaxed`（内部摘要调用，不经过用户可见路径）
- 20 秒硬超时，失败不阻塞正常流程
- 摘要保留关键话题、核心需求和重要结论，省略寒暄和工具细节
...
conversation.mode
    │
    ├─ "daily" ───── detect_activation()
    │                 ├─ 有 Skill 激活 + 有文件 → 直接激活 → confirming
    │                 ├─ 有 Skill 激活 + 无文件 → DailyChat（LLM 自然引导）
    │                 └─ 无 → DailyChat
    │
    ├─ "confirming" ── is_daily_question()?
    │   │               ├─ 是 → DailyChat（不改 mode）
    │   │               └─ 否 → Skill.on_step_complete()
    │   │                        ├─ AdvanceToStep → 进入 analyzing
    │   │                        └─ Abort → clear_analysis_notes() → daily
    │   │
    └─ "analyzing" ─── is_daily_question()?
                        ├─ 是 → DailyChat（不改 mode）
                        └─ 否 → Skill.on_step_complete()
                                 ├─ AdvanceToStep → 推进到下一步
                                 ├─ WaitForUser → 重跑当前步
                                 ├─ Finish → clear_analysis_notes() → daily
                                 └─ Abort → clear_analysis_notes() → daily
```
...
| 3 | `load_file` | 加载上传文件，数据自动注入 execute_python 环境（`_df`/`_text`）。同会话重复加载同一文件时命中缓存，跳过重新解析 | `file_id`*, `sheet`, `nrows` |
| 4 | `generate_report` | 生成 HTML/PDF/DOCX/Markdown 报告 | `title`*, `sections`*[], `format` |
| 5 | `generate_chart` | 交互式数据可视化 Plotly HTML（bar/line/scatter/box/heatmap/pie/histogram） | `chart_type`*, `title`*, `data`, `options` |
...
| 9 | `export_data` | 导出数据（CSV/Excel/JSON）。两种输入模式：`source_file`（从现有文件转换格式）或 `data`（JSON records 直接导出）。DataFrame 导出推荐在 execute_python 中使用 `_export_detail()` | `format`*, `filename`*, `source_file`, `data` |
...
Skill 的 `workflow.toml` 中 `tools_only` 定义每步可用工具。过滤分两层：

1. **Schema 层**（LLM 可见性）：`tool_defs_override` 仅向 LLM 暴露当前步骤允许的工具 Schema，LLM 不知道其他工具的存在
2. **执行层**（运行时强制）：`agent_loop` 在调用 `tool_registry.execute()` 前检查 `allowed_tools: HashSet<String>`，阻断 LLM 幻觉出的不在步骤工具集中的工具调用

```
Step 0 (方向确认):    load_file, save_analysis_note
                      ── 极小工具集，防止 Agent 提前清洗数据

Step 1 (数据清洗):    load_file, execute_python,
...
日常模式:             6 个工具（排除分析专用）
                      web_search, execute_python, load_file,
                      generate_report, generate_chart, export_data
                      ── ToolFilter::Exclude 排除 hypothesis_test,
                         detect_anomalies, save_analysis_note,
                         update_progress（~500 token 节省）
```
...
当 LLM 在单次响应中返回多个 Tool Call 时，`agent_loop` 使用 `futures::future::join_all` 并行执行，然后按原始顺序处理结果：

```
LLM 返回 N 个 Tool Calls
    │
    ├─ Phase 1: 预处理（顺序）
    │   ├─ emit tool:executing 事件
    │   └─ 执行时工具过滤（blocked → 直接返回错误）
    │
    ├─ Phase 2: 执行（并行）
    │   ├─ N=1 → 直接 await（零开销）
    │   └─ N>1 → join_all 并发执行 → 按原始索引排序
    │         └─ 日志记录: 并行耗时 vs 顺序耗时估算
    │
    └─ Phase 3: 后处理（顺序）
        ├─ file:generated 事件
...
**安全性**：`PluginContext` 实现 `Clone`（所有字段为 `Arc` 或可 Clone 类型），每个并发任务持有独立的 context 引用。LLM 在同一批次中生成的 Tool Call 彼此独立（不依赖对方的结果），因此并行执行不引入竞态条件。
...
### 6.5 load_file 缓存与多文件 Preamble

**缓存机制**：`handle_load_file` 在解析文件前检查 `loaded:{conv_id}:{file_id}` memory key。命中缓存时直接返回已有元数据（`"cached": true`），跳过耗时的 Python 文件解析。

**多文件 Preamble 注入**：`build_loaded_files_preamble()` 为 `execute_python` 自动注入文件加载代码：
- 单文件：直接赋值 `_df` 或 `_text`
- 多文件：使用 `_dfs` / `_texts` 字典，以 `file_id`（UUID）为 key 避免同名文件冲突，`_df` / `_text` 指向最后加载的文件
- 路径安全：所有路径经 `canonicalize()` + `starts_with(workspace)` 校验，防止路径穿越
...
Skill::system_prompt(state) → String
    ...
    └─ Legacy 路径（仅无 Skill 匹配时）
        ├─ SYSTEM_PROMPT_BASE   prompts/base.md（~28 行，4 级结构）
        │   · 角色人格："靠谱的同事"+ 组织咨询专家
        │   · 【核心规则】数据真实性 / 联网搜索 / 文件工具 / 保密
        │   · 【文件处理】load_file → _df/_text → execute_python
        │   · 【工作方式】行动导向决策树
        │   · 【Python 环境】可用库 + helper 函数（1 行）
        │
        └─ 模式专属提示词 + 日期注入
             │
...
send_message()
    │
    ├─ gateway.set_busy(conversation_id)    ← 入口处立即占位（TOCTOU 防护）
    │                                          所有后续早退路径调用 clear_task()
    ├─ 保存用户消息 + 加载设置 + 路由        ← 占位保护区间
    │
    ├─ insert_active_task()                  ← 记录到 DB（崩溃恢复用）
    │
    ├─ tokio::spawn:
    ...
    │         ├─ remove_lock_with_retry()     ← 清理 run.lock（1 次重试）
    │         └─ gateway.clear_task()         ← 释放槽位
    │
    * 如果 agent_loop panic:
      AgentGuard::Drop
         ├─ remove_lock_with_retry()         ← 同步执行（1 次重试）
         └─ gateway.clear_task()             ← 同步清理（std::sync::Mutex）
```

**run.lock 清理重试**：`remove_lock_with_retry()` 尝试删除 run.lock 文件，失败后等待 50ms 重试一次。第二次仍失败则记录 CRITICAL 日志，依赖下次启动时孤儿检测清理。
...
`gateway.rs` 对所有 LLM 请求（流式和非流式）实施自动重试：

```
dispatch_stream() / dispatch_send()
    │
    └─ retry_dispatch_stream() / retry_dispatch_send()
         │
         ├─ 成功 → 返回结果
         │
         └─ 失败 → is_retryable_error()?
              │
              ├─ 可重试（429/5xx/timeout/connection） → 等待退避 → 重试
              │   · 最多 3 次重试（共 4 次尝试）
              │   · 退避: 1s → 2s → 4s（指数增长）
              │   · 抖动: +0~25% 随机延迟（防雷群效应）
              │
              └─ 不可重试（401/400/403/未知） → 立即返回错误
```

**可重试错误**：HTTP 429（限流）、500/502/503/504（服务器错误）、网络超时/连接拒绝/断连

**不可重试错误**：HTTP 401/403（认证错误）、400（请求格式错误）、其他未知错误
...
| 事件名 | 触发时机 | 载荷 | 发射者 |
|--------|----------|------|--------|
| `streaming:delta` | LLM 输出 token | `{ conversationId, delta }` | `agent_loop` |
| `streaming:done` | Agent Loop 完成 | `{ conversationId, messageId }` | **仅 `AgentGuard::clear()`** |
| `streaming:error` | 请求失败 | `{ conversationId, error }` | `agent_loop` |
| `streaming:step-reset` | 步骤切换 | `{ conversationId, step }` | `chat.rs` |
| `tool:executing` | 开始执行工具 | `{ conversationId, toolName, toolId, purpose }` | `agent_loop` |
...
send_message() 启动 Agent 时:
    会话目录/run.lock ← 写入 "SESSION_UUID:UNIX_TIMESTAMP"

AgentGuard::clear() 完成时:
    会话目录/run.lock ← 删除
```

`AppStorage` 在初始化时生成一个唯一的 `session_id`（UUID v4），所有 run.lock 写入该 session_id 而非 PID。这避免了 PID 重用问题——即使 OS 在崩溃后将相同 PID 分配给新进程，session UUID 仍然不同。
...
```
应用启动（新 session_id 生成）
    │
    ▼ 扫描 conversations/*/run.lock
    │
    ├─ 文件存在？
    │    │
    │    ├─ 解析 "SESSION_ID:TIMESTAMP"
    │    │    │
    │    │    ├─ session_id != 当前 session_id → 孤儿（崩溃+重启）
    │    │    │         │
    │    │    │         ├─ 1. 读取 analysis.json
    │    │    │         ├─ 2. 重置 "in_progress" 步骤状态为 "paused"
    │    │    │         └─ 3. 删除 run.lock 文件
    │    │    │
    │    │    ├─ session_id == 当前 session_id → 正常（当前会话的任务）
    │    │    │
    │    │    └─ 时间戳超过 24 小时 → 孤儿（安全兜底）
    │    │
    │    └─ 文件格式异常/读取失败 → 视为孤儿，执行清理
    │
    └─ 文件不存在 → 跳过
```
...
作为本地桌面应用，安全策略以**入口静态校验**为核心，不使用运行时 import hook（曾因误杀合法库依赖导致 openpyxl/shutil/urllib.parse 反复出问题）。
...
**静态检查禁止的模块**（`validate_code` 入口检查）：
```
subprocess, importlib, multiprocessing
```

> 本地桌面环境已放宽：`shutil`, `requests`, `socket`, `http`, `urllib` 不再禁止（数据分析常用的文件操作和网络访问）。写路径限制仍保护 workspace 安全。

**静态检查禁止的函数调用**：
```
exec(), eval(), compile()             — 独立调用检测，pd.eval()/re.compile() 等方法调用不受影响
__import__(), builtins.__import__()
_real_import(), _safe_import._real()
_original_open(), _safe_open._orig()
os.system(), os.popen(), os.exec*(), os.spawn*(), os.posix_spawn(), os.fork()
import ctypes, from ctypes
```

> `exec()`/`eval()`/`compile()` 使用**独立调用检测**（`contains_standalone`）：仅当调用前无 `.`/标识符字符时才拦截。`pd.eval('expr')`、`df.eval('A>B')`、`re.compile(pattern)` 等方法调用正常放行。
...
_ALLOWED_PATHS = ['{workspace_path}', ...]  # workspace + uploads/exports/reports/charts/analysis/temp
os.chdir(_ALLOWED_PATHS[0])

# Enforce memory limit (Unix only)
...
from scipy import stats as scipy_stats  # 统计检验
```

**Part 3（静态 — 文件写限制）**：
```python
# File write restriction — blocks writes outside workspace
...
def _safe_open(file, mode='r', *args, **kwargs):
    if isinstance(file, (str, bytes)):
        ...
        if any(m in str(mode) for m in ('w', 'a', 'x')):
            ...
            if not any(abs_path.startswith(os.path.realpath(p)) for p in _ALLOWED_PATHS):
                raise PermissionError(f"Writing to '{file_str}' is blocked")
    return _original_open(file, mode, *args, **kwargs)
builtins.open = _safe_open
```

> 注意：无运行时 import hook。所有模块导入限制由 `validate_code()` 静态检查在执行前完成。
...
| `_smart_read_data(path)` | CSV/Excel/JSON/Parquet 自动分发 + 编码探测 + 列名清洗 |
...
分析模式下（`orchestrator::get_step_state` 有值），`handle_execute_python` 在 preamble 之后、LLM 代码之前额外注入两部分：

1. **分析 preamble**：`_CONV_ID`、`_ANALYSIS_DIR`、`_CURRENT_STEP`、三层 DataFrame 快照管理、`_df_raw` 只读引用
2. **预写分析函数**（`python/analysis_utils.rs:ANALYSIS_UTILS`，~700 行 Python）

**三层 DataFrame 快照**：

| 层 | 文件 | 时机 | 用途 |
|----|------|------|------|
| Layer 1 | `_original.pkl` | 首次加载时保存，永不修改 | 回退到原始数据 |
| Layer 2 | `_step{N}_df.pkl` | 每次 execute_python 后保存（按当前步骤编号） | 按步回退 |
| Layer 3 | `_step_df.pkl` | 每次 execute_python 后保存 | 跨调用持久化工作数据 |

运行时变量：`_df_raw`（原始数据只读副本）、`_df`（当前工作数据，每步自动更新）、`_CURRENT_STEP`（当前步骤编号）。

注入顺序：
```
一次性模式（日常）：
  [sandbox preamble]       ← 安全限制 + pandas/numpy/scipy 导入 + 基础工具
...
**快照清理**：用户上传新文件时，`handle_load_file` 清除 `analysis/{conv_id}/` 下所有 `.pkl` 快照，确保新数据从头开始。分析完成/取消时，`clear_analysis_notes()` 删除整个目录。

**预写函数清单**（LLM 只需调用，无需编写）：

| 函数 | 功能 | 调用场景 |
|------|------|----------|
| `_detect_columns(df)` | 列名语义检测（13 种标准字段 + 10 种薪酬组成），返回 `{detected, salary_components, undetected}` | Step 1 |
| `_normalize_salary(series)` | 处理万/K/货币符号/百分号 → float | Step 1 |
| `_step1_clean(df, col_map)` | 完整清洗流程（5 条排除规则 + 薪酬结构 + 数据质量），自动更新 `_df` | Step 1 |
| `_step4_diagnose(df, col_map)` | 六维度公平性诊断（调用 dim1~dim6） | Step 4 |
| `_step5_scenarios(df, col_map, diagnosis)` | 三档调薪方案 + ROI 测算 | Step 5 |
| `_dim1_internal_equity` | 岗位内部公平性（CV/极差比/IQR） | Step 4 内部 |
| `_dim2_cross_position` | 跨岗位公平性（中位数偏离） | Step 4 内部 |
| `_dim3_regression` | 薪酬-司龄回归分析（OLS + 残差异常） | Step 4 内部 |
| `_dim4_inversion` | 薪酬倒挂检测（新 vs 老员工中位数） | Step 4 内部 |
| `_dim5_structure_fit` | 固浮比 vs 岗位性质匹配 | Step 4 内部 |
| `_dim6_compa_ratio` | CR 分布分析（5 档标记） | Step 4 内部 |
| `_calc_cv`, `_calc_gini`, `_calc_compa_ratio` | 统计辅助函数 | 内部 |
| `_detect_outliers_iqr`, `_salary_stats`, `_tenure_years` | 统计辅助函数 | 内部 |
| `_cache_result(step_key, data)` / `_load_cached(step_key)` | 结果缓存到 `analysis/{conv_id}/stepN_results.json` | 跨步骤 |
| `_preview(n)` | 快速预览当前 `_df` 形状和前 N 行 | 随时 |
| `_data_status()` | 显示原始 vs 当前行数差异、可回退步骤 | 随时 |
| `_reset_to_original()` | 回退到原始数据 | 用户要求 |
| `_reset_to_step(step_num)` | 回退到指定步骤的数据 | 用户要求 |
| `_export_current(filename, title, format)` | 导出当前 `_df` 到文件（解决 export_data 工具不使用 `_df` 的问题） | 用户要求导出 |

**设计理由**：国产 LLM（DeepSeek 等）运行时现写 Python 分析代码质量不稳定。开发时由 Claude Code 预写可靠的分析代码，打包进应用；运行时 LLM 只需调用预写函数，偶尔补充少量代码。日常模式不注入，零开销。三层快照 + 自动 `_df` 更新确保数据流不依赖 LLM 正确赋值。

### 13.5 Windows GBK 编码兼容
...
### 13.7 持久 REPL 会话（分析模式）

分析模式（Step 0-5）使用持久 Python REPL 进程（`python/session.rs`），消除每次 `execute_python` 的冷启动开销（进程启动 + import + 编译 + pickle 反序列化）。日常模式仍使用一次性 `PythonRunner`。

**双路径执行**（`tool_executor/python.rs`）：
```rust
if is_analysis {
    // 持久 REPL：热进程，变量在内存中存活
    ctx.session_manager.execute(&conv_id, &code, 120s, &sandbox).await
} else {
    // 一次性进程：无状态，30s 超时
    PythonRunner::with_config(workspace, sandbox).execute_raw(&code).await
}
```
...
meta:    {workspace}/temp/_meta_{uuid}.json
```

> `__GENERATED_FILE__` 标记不在 REPL 内处理，直接透传 stdout 给 Rust 侧统一解析。

**PythonSessionManager**（`session.rs`）：

| 参数 | 值 | 说明 |
|------|----|------|
| 最大并发会话 | 3 | 每个活跃分析会话一个 Python 进程 |
| 空闲超时 | 15 分钟 | 定时回收，5 分钟检测间隔 |
| 执行超时 | 120 秒 | 单次代码块超时 |
| stdout 上限 | 1 MB | 防止 OOM，超限后丢弃但继续读取至 `__DONE__` |
| LRU 淘汰 | 满 3 会话时淘汰最久未用 | 后台 checkpoint + kill |

**会话生命周期**：
```
spawn → initialize（sandbox preamble + analysis_utils）→ [execute ↔ execute ...] → checkpoint → kill
                                                          ↑
...
| 防护点 | 机制 | 位置 |
|--------|------|------|
| Python 代码中的文件路径 | `py_escape()` 转义 `\` → `\\`、`'` → `\'`、`\n` → `\\n`、`\r` → `\\r`，使用普通 `''` 字符串（非 `r''`） | `parser.rs`, `tool_executor/util.rs` |
| `__GENERATED_FILE__` 注册 | `canonicalize()` + `starts_with(workspace)` 阻止路径穿越 | `tool_executor/python.rs` |
| `__GENERATED_FILE__` JSON 标记 | `json.dumps()` 序列化（防 title/filename 注入） | `sandbox.rs` `_export_detail` |
| `_export_detail` 文件名 | `os.path.basename()` 剥离目录组件（防路径穿越写入） | `sandbox.rs` `_export_detail` |
| Python `open()` 写操作 | `_safe_open` Hook：写模式（w/a/x）限制在 `_ALLOWED_PATHS` 内，读操作不限制 | `sandbox.rs` preamble |
...
- **TOCTOU 防护**：`send_message` 在入口处立即调用 `set_busy()`（原子占位），防止双击导致重复保存用户消息。所有后续早退路径均调用 `clear_task()` 释放占位
...
- **消息分片原子性**：`_current` 元数据使用 write-tmp-then-rename 原子写入，读取时自动回退 `.tmp` 文件（防崩溃丢失元数据）

### 15.4 工具结果可靠性（4 层保障）

文件生成工具（`generate_report`/`generate_chart`/`export_data`）可能降级（如 PDF→HTML），但 LLM 忽略降级通知仍告诉用户错误格式。通过 4 层防御确保用户看到真实文件信息：

```
工具执行完成
    │
    ├─ Layer 1: FileMeta 结构化元数据
    │   ToolOutput 携带 FileMeta（文件格式、降级标志）
    │   → 3 个文件工具通过 FileGenResult 返回
    │
    ├─ Layer 2: 系统直达通道（绕过 LLM）
    │   ├─ file:generated 事件 → 前端 toast 降级警告
    │   └─ finish_agent → generatedFiles JSON 注入降级信息
    │       → GeneratedFileCard 显示橙色 badge + 降级提示
    │
    ├─ Layer 3: 结构化标签（辅助 LLM）
    │   [TOOL_STATUS: degraded]
    │   [REQUESTED_FORMAT: pdf]
    │   [ACTUAL_FORMAT: html]
    │   → 注入工具结果消息，帮助 LLM 如实描述
    │
    └─ Layer 4: 事后校验（最终防线）
        verify_file_claims() 扫描 LLM 文本
        → 格式声明词 + 动作词 vs actual_format
        → 不匹配时追加修正脚注
```

**Layer 1: ToolOutput 结构扩展**

`ToolOutput`（`plugin/tool_trait.rs`）新增 3 个字段：

| 字段 | 类型 | 说明 |
|------|------|------|
| `is_degraded` | `bool` | 是否降级（默认 `false`） |
| `degradation_notice` | `Option<String>` | 降级说明文本 |
| `file_meta` | `Option<FileMeta>` | 文件元数据（格式、大小、路径） |

`FileMeta` 结构体包含 `file_id`、`file_name`、`requested_format`、`actual_format`、`file_size`、`stored_path`、`category`。

3 个文件工具的 handler（`tool_executor/` 子模块）返回 `FileGenResult`，由 Plugin Wrapper 映射到 `ToolOutput`。

**Layer 2: 系统直达 `file:generated` 事件**

Agent loop 中工具执行后立即发射 `file:generated` 事件（不经过 LLM），前端直接展示真实文件信息：
- 降级时弹 warning toast（10 秒自动消失）
- `GeneratedFileCard` 显示橙色边框 + 图标角标 + 降级提示条
- `finish_agent` 保存消息时注入 `isDegraded`/`degradationNotice`/`requestedFormat` 字段

**Layer 3: 结构化 Tool Result 标签**

`tag_file_tool_result()` 在工具结果送入 LLM 消息历史前注入结构化标签（`[TOOL_STATUS]`/`[FORMAT]`/`[FILE]`/`[NOTICE]`），帮助 LLM 正确报告实际格式。

**Layer 4: Post-stream 文件声明校验**

`verify_file_claims()` 在 `finish_agent` 前扫描 LLM 输出文本：
1. 查找格式声明词（PDF/DOCX/Excel）— 大小写不敏感，在 lowercased 文本中搜索
2. 近距离（60 字符窗口）检测动作词（已生成/已导出/generated/exported…），中文在原始文本搜索，英文在 lowercased 文本搜索（避免 Unicode `to_lowercase()` 字节偏移不一致）
3. 排除否定上下文（40 字符窗口：失败/不可用/failed/unavailable…）
4. 与 `all_file_metas` 的 `actual_format` 交叉比对
5. 格式族去重（Excel/XLS/XLSX 视为同一族）
6. 不匹配时追加修正脚注（`> **更正**：实际生成的文件格式为 **HTML**（非 PDF）`）
...
| Skill 直接激活（有文件时） | `commands/chat.rs:send_message`（daily 分支） |
| 日常问题检测 | `commands/chat.rs:is_daily_question` |
| 分析 notes 清理 | `commands/chat.rs:clear_analysis_notes` |
...
| 日常模式上下文压缩 | `commands/chat.rs:compress_context_if_needed`（非流式 LLM 摘要，20s 超时） |
| LLM 流式请求 | `llm/gateway.rs:stream_message` |
| LLM 请求重试（指数退避） | `llm/gateway.rs:retry_dispatch_stream` / `retry_dispatch_send` |
| 可重试错误判定 | `llm/gateway.rs:is_retryable_error`（429/5xx/timeout/connection） |
...
| 分析 preamble + epilogue | `llm/tool_executor/python.rs:handle_execute_python`（三层快照 + `_df_raw` + `_CURRENT_STEP` + `ANALYSIS_UTILS`） |
| HTML 报告生成 | `plugin/builtin/tools/report_gen.rs`（wrapper），`llm/tool_executor/report.rs:handle_generate_report`（实现） |
| 崩溃恢复扫描 | `lib.rs`（启动时） |
| Agent 守卫清理 | `commands/chat.rs:AgentGuard::clear` / `Drop` |
| 路径穿越校验 | `llm/tool_executor/python.rs:handle_execute_python` (`__GENERATED_FILE__`) |
| 运行时 Import Hook | 已移除（静态校验 `validate_code` 替代），参见 §13.1 设计决策 |
| 运行时 File Write Hook | `python/sandbox.rs:preamble` (`_safe_open`，写操作限制 `_ALLOWED_PATHS`，原始 open 存于 `_original_open`) |
| 临时文件清理 | `lib.rs:cleanup_temp_dir` |
| 搜索（SearXNG） | `search/searxng.rs` |
| 搜索（Bocha） | `search/bocha.rs` |
| 搜索（Tavily） | `search/tavily.rs` |
| 文件元数据结构 | `plugin/tool_trait.rs:FileMeta` |
| 文件生成结果结构 | `llm/tool_executor/mod.rs:FileGenResult` |
| 工具结果结构化标签 | `commands/chat.rs:tag_file_tool_result` |
| LLM 文件声明校验 | `commands/chat.rs:verify_file_claims` |
| file:generated 事件发射 | `commands/chat.rs:agent_loop`（工具执行后） |
| 降级信息注入 generatedFiles | `commands/chat.rs:finish_agent` |
| TOCTOU 防护（早期占位） | `commands/chat.rs:send_message`（`set_busy` 在入口处，早退 `clear_task`） |
...
| 多文件 Preamble 注入 | `llm/tool_executor/file_load.rs:build_loaded_files_preamble`（`_dfs`/`_texts` 字典，file_id 为 key） |
| 路径转义函数 | `llm/tool_executor/util.rs:py_escape`（`pub(crate)`，`parser.rs` 共用） |
...
| `MAX_RETRIES` | 3 | `gateway.rs` | LLM 请求最大重试次数（429/5xx/timeout） |
| `INITIAL_BACKOFF_MS` | 1000 | `gateway.rs` | 重试初始退避延迟（ms），每次翻倍 |
...
| `COMPRESS_THRESHOLD_CHARS` | 24000 | `chat.rs` | 日常模式上下文压缩触发阈值（字符） |
| `COMPRESS_KEEP_RECENT` | 10 | `chat.rs` | 压缩时保留的最近消息数 |
...
| 分析冷却期 | 60s | `chat.rs` | 分析完成后跳过 detect_activation |
| `EXTRACT_TIMEOUT` | 30s | `checkpoint.rs` | 检查点提取超时 |
| `STALE_STREAM_TIMEOUT_MS` | 200s | `useStreaming.ts` | 前端流式活动超时（看门狗），首次发现时初始化时间戳而非立即清除 |
| `WATCHDOG_INTERVAL_MS` | 10s | `useStreaming.ts` | 看门狗检查间隔 |
