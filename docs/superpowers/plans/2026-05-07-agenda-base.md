...
**Review 修正（code quality）：create 输入校验 / 规范化**
...
```bash
cd src-tauri && cargo test --lib transport::tauri_commands::agenda::tests
```

期望：FAIL，提示 `build_agenda_item_from_create_request` 未定义。

- [ ] **Step R3：实现**

在 `create_agenda_item` 上方追加私有 helper，并让命令调用 helper：
...
git commit -m "fix(agenda): validate create command input"
```
