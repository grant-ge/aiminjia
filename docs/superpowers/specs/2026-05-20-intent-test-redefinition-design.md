...
| **验收标准** | 是 | 用两种 bullet 组织：`✅ 应该看到` 列 PASS 条件、`❌ 不应该看到` 列必须主动检查的反向陷阱；至少要有 `✅` 条目（详见 §2.4） |
...
- **没有「判定提示」段**——它原本想塞的 3 类内容各有更好的归宿：边界容忍（如「`nextFireAt` 比 `startAt` 早 1-2 秒是合法误差」）→ 写进验收标准的容忍范围（「在 T0 ± 1 分钟内」）；通用诊断套路（如「先用 `aijia where --json` 看现场」）→ 写进 `test-intents` skill；反向陷阱 → 写进 `❌ 不应该看到`。留判定提示段会鼓励作者偷懒、绕开「验收标准要精确」的要求。
- **「验收标准」内部用 ✅ / ❌ 区分正反**——正反断言是同一判定逻辑的两面，合并到一段视觉上集中、不易漏；`❌ 不应该看到` 段可以为空但不能没有这段头部（提醒作者想反向陷阱）。
...
`❌ 不应该看到` 段的每条 bullet **本身就是反向**——agent 主动检查「X 不存在 / X 没出现」：
...
```
❌ JSON 字段都正确（这是正向断言，应该放 ✅ 段）
❌ 不应该出错（太模糊，agent 不知道查什么）
```

`❌ 不应该看到` 段允许为空（写「无」），但段头必须保留，提醒下次修订者主动想反向陷阱。
...
- ✅ 应该看到：表单收起 ✓ / 列表出现条目 ✓ / `agenda-uuid.json` 字段全对 ✓
- ❌ 不应该看到：无旧字段名 ✓ / 列表无重复 ✓
- 备注：无
