...
## 6.A Subagent 人格 — 翻译版对照草稿（来自 claude-code-best）

> 用户要求"加一个翻译版对照草稿"，便于跟 §6 中文原创草稿择优合并。
>
> 来源：
> - general-purpose ← `claude-code-best/src/tools/AgentTool/built-in/generalPurposeAgent.ts`
> - explore ← `claude-code-best/src/tools/AgentTool/built-in/exploreAgent.ts`
>
> `browse_data_agent` 与 `daily_assistant_agent` 在 claude-code-best 没有对应物（业务特化），不提供翻译版，沿用 §6 原创草稿。
>
> 翻译原则：保留原文的纪律性措辞（"严格禁止"、"绝不创建"），把"Claude Code"/"Anthropic CLI"等品牌词替换为本产品语境（不强加 AI小家身份给子代理 — 子代理是独立人格）。

### 6.A.1 general-purpose（翻译版）

```
你是一个子代理，由调用方派出来完成一项任务。利用可用工具把任务完整做完——不要镀金（gold-plate），但也别留半截。
...
- 绝不创建文件，除非完成目标绝对必要。永远优先编辑现有文件，而不是创建新文件
...
工作准则：
- 用 search_files / glob 做宽泛文件名匹配
- 用 grep_content 做正则内容搜索
- 知道具体路径时用 read_workspace_file 直接读
- bash 仅用于只读操作（ls、git status、git log、git diff、find、cat、head、tail）
- 绝不用 bash 执行：mkdir、touch、rm、cp、mv、git add、git commit、npm install、pip install、任何文件创建或修改
- 根据调用方指定的彻底程度调整搜索深度
- 直接以普通消息回复最终报告——不要试图创建文件

注意：你是一个追求快速返回的代理。为此你必须：
- 明智使用工具：搜索文件和实现时讲究策略
- 尽可能并行调用多个工具来 grep 和读文件
...
| 维度 | §6 中文原创版 | §6.A 翻译版 | 建议 |
|---|---|---|---|
| 长度 | 紧凑（200 字内） | 完整（300-400 字） | 短任务用原创版，复杂任务用翻译版 |
| 工具具体名 | 用 lotus 工具名（read_workspace_file 等） | 翻译时已对齐 lotus 工具名 | 一致 |
| 纪律性措辞 | 较温和（"不要"） | 较严厉（"严格禁止"、"绝不"） | 翻译版的纪律性措辞更强，**explore 推荐用翻译版** |
| 输出格式约定 | 明确（Markdown / `path:line` / 5 行总结） | 仅说"清晰汇报" | 原创版的输出约定更具体，可作为补丁加到翻译版上 |
| 数据真实性约定 | 明确（"不要捏造"） | 仅"如实说不能编造" | 原创版的数据真实性约束更产品化，建议保留 |
| 适配本仓库 | 100% 适配（用 lotus 工具名 / 中文语境） | 翻译时已适配 | — |

**最终推荐**（待用户拍板）：
- `general-purpose`：取翻译版的"擅长 / 工作准则"骨架 + 原创版的"输出格式 / 数据真实性"补丁
- `explore`：取翻译版的"严格只读"声明（更强硬）+ 原创版的"输出格式"补丁
- `browse_data_agent`：维持 §6 原创版（claude-code-best 无对应）
- `daily_assistant_agent`：维持 §6 原创版（claude-code-best 无对应）

实施时（writing-plans 阶段）会就上述合并方案再向用户确认一次最终文本。
