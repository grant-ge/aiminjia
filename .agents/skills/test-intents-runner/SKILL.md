...
| 任何步骤挂 | **验收永远要跑**——挂在哪一步都核验 `应该看到` / `不应该看到` 段，看现场磁盘 / UI 状态对 triage 有用 |
...
- 登录账号选择顺序：已有有效登录态且意图不要求重新登录时沿用当前账号；当前未登录或意图必须走登录表单时，优先用用户本轮提供的账号密码；只有用户未提供账号密码且不登录就无法继续跑时，才用兜底账号 `test@ai` / `testtest`，不要停下来反问；报告里只写账号和 scope，不重复输出密码
