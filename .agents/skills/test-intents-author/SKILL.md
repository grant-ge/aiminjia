...
| **验收标准** | 一组 bullet，逐条列出可观察、可复验的判定项；不拆正反二分或结果二分小段 |
...
```
反例（复合）：日程已创建且能被 cron 调度（说明 active）
正例（拆开）：
  - 列表出现一行标题为 `早会提醒` 的条目
...
```
反例（推测）：agenda 相关目录下
正例（字面）：~/.renlijia/users/{scope}/agenda/items/
正例（模式）：~/.renlijia/users/{scope}/agenda/items/agenda-*.json
```
...
这些都属于验收标准，直接放在同一组 bullet 里。不要另起任何二级小段；没有反向约束时也不要写「无」占位。
...
| 冗余 | 「列表出现 1 条目」+「列表长度 == 1」——同一断言两种说法 |
| 包含关系 | 「文件 X 存在」+「字段 X 合法」——后者隐含前者，删冗 |
| 标准之间冲突 | 同一组验收标准里同时写 `status == "active"` 和 `status != "paused"`——后者被前者覆盖，容易制造伪约束 |
...
| 「我把验收标准拆成通过/失败两段，读起来更清楚」 | 不拆段；所有判定项都放在 `验收标准` 的同一组 bullet 里（§5 规则 6） |
