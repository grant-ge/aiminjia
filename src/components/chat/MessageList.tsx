...
export function MessageList({ expertTeamId }: MessageListProps = {}) {
              ...
              renderInterleavedBlocks(
                t.blocks,
                {
                  ...
                  inlineStreamingContent:
                    i === turns.length - 1 && showStreamingBubble && streamingContent
                      ? streamingContent
                      : null,
  ...
  function renderInterleavedBlocks(
    ...
    const firstTextIso =
      blocks.find((b): b is Extract<RenderTurnBlock, { kind: 'assistantText' }> => b.kind === 'assistantText')
    ...
    const renderBlock = (b: RenderTurnBlock, idx: number) => {
      ...
      if (b.kind === 'teamMarker') {
        ...
        return (
          <TeamVisualProvider key={`team-${b.toolCallId}`} value={ctx.expertTeam ?? null}>
            <TeamProgressBlock session={ctx.teamSession} onOpen={ctx.onOpenTeamDrawer} />
          </TeamVisualProvider>
        )
      }
      return null
    ...
    const walkAndGroup = (slice: RenderTurnBlock[], keyPrefix: string, baseIdx: number): ReactNode[] => {
