...
describe("design.pen token alignment", () => {
  ...
  it("sets pointer cursor for enabled buttons globally", () => {
    expect(CSS).toMatch(
      /button:not\(\s*:disabled\s*\)\s*\{[^}]*cursor:\s*pointer;/s,
    );
  });
  ...
  it("uses a crisp, low-blur composer outline instead of the generic card shadow", () => {
    expect(tokenValue("--composer-border")).toBe("rgba(13, 13, 13, 0.16)");
    expect(tokenValue("--shadow-composer")).toBe(
...
describe("assistant markdown typography", () => {
  it("restores markdown heading, list, and rich text styles inside the assistant scope", () => {
    ...
    expect(CSS).toMatch(
      /\.assistant-markdown ul\s*\{[\s\S]*list-style-type:\s*disc;/,
    );
    expect(CSS).toMatch(
      /\.assistant-markdown ol\s*\{[\s\S]*list-style-type:\s*decimal;/,
    );
    expect(CSS).toMatch(
      /\.assistant-markdown blockquote\s*\{[\s\S]*border-left:/,
    );
    expect(CSS).toMatch(
      /\.assistant-markdown :not\(pre\) > code\s*\{[\s\S]*background:\s*var\(--color-bg-code\);/,
    );
    expect(CSS).toMatch(
    ...
    expect(CSS).toMatch(
      /\.assistant-markdown \.markdown-table-copy\s*\{[\s\S]*font-size:\s*inherit;/,
    );
    expect(CSS).not.toMatch(
      /\.assistant-markdown \.markdown-table-copy\s*\{[\s\S]*font-size:\s*15px;/,
    );
    expect(CSS).toMatch(
      /\.assistant-markdown \.markdown-table-scroll > table\s*\{[\s\S]*border-collapse:\s*collapse;/,
    );
    expect(CSS).toMatch(
      new RegExp(
        "\\.assistant-markdown \\.markdown-table-scroll th,\\s*\\.assistant-markdown \\.markdown-table-scroll td\\s*\\{[\\s\\S]*height:\\s*40px;",
      ),
    );
