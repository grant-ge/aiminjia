...
describe('MonitoringUrlsForm', () => {
  it('renders one empty row when initial value is empty', () => {
    ...
    expect(screen.getAllByPlaceholderText(/监测对象名称/)).toHaveLength(1)
  })
  ...
  it('preserves existing rows when initial has monitoringTargets', () => {
    ...
    expect(screen.getAllByPlaceholderText(/监测对象名称/)).toHaveLength(2)
  })
  ...
  it('calls onSubmit with normalized monitoringTargets on save', () => {
    ...
    fireEvent.change(screen.getAllByPlaceholderText(/监测对象名称/)[0], { target: { value: 'Anthropic' } })
    fireEvent.change(screen.getAllByPlaceholderText(/https/)[0], { target: { value: 'https://anthropic.com' } })
