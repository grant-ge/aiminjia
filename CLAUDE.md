...
发布新版本分两步：GitHub Actions 自动构建 + OSS 上传。

### Step 1: 打 Tag 触发 CI 构建

```bash
# 1. 更新版本号
#    - code/package.json → version
#    - code/src-tauri/Cargo.toml → version
#    - code/src-tauri/tauri.conf.json → version
...
GitHub Actions（`.github/workflows/build-desktop.yml`）自动：
- 构建 macOS ARM `.dmg`（macos-14）
- 构建 Windows `.exe`（windows-latest）
- 创建 GitHub Release（Draft）并上传产物
- macOS Intel `.dmg` 需本地交叉编译：`cd code && pnpm tauri build --target x86_64-apple-darwin`

### Step 2: 上传到阿里云 OSS

```bash
# 上传所有安装包到 OSS（从 GitHub Release 下载 + 本地 Intel DMG）
cd code && python3 scripts/upload-to-oss.py X.Y.Z
```

脚本自动：
- 从 GitHub Release 下载 ARM DMG + Windows EXE
- 读取本地 Intel DMG（交叉编译产物）
- 上传到 `aijia/vX.Y.Z/` 版本目录
- 复制到 `aijia/latest/` 目录（Landing Page 下载链接指向此处）

### Step 3: 更新 Homebrew Cask
...
| `code/scripts/upload-to-oss.py` | OSS 上传脚本 |
