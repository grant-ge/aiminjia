...
git tag vX.Y.Z
git push codeup main --tags
```

GitHub Actions CI（`.github/workflows/build-desktop.yml`）：
- macOS ARM `.dmg`（macos-14 runner）
- Windows `.exe`（windows-latest runner）
- ��建 GitHub Release（Draft）

> **注意**: GitHub push 可能被 Push Protection 阻断（历史 commit 中的 AK/SK），Codeup 不受影响。

### Step 3: 本地构建 macOS DMG（ARM + Intel）

```bash
cd code && pnpm tauri build                                  # ARM (aarch64)
cd code && pnpm tauri build --target x86_64-apple-darwin     # Intel (x86_64)
```

产物位置：
- ARM: `code/src-tauri/target/release/bundle/dmg/AIjia_X.Y.Z_aarch64.dmg`
- Intel: `code/src-tauri/target/x86_64-apple-darwin/release/bundle/dmg/AIjia_X.Y.Z_x64.dmg`

> Intel 交叉编译 DMG 打包可能失败（已知 Tauri 问题），手动补救：
> `cd code/src-tauri/target/x86_64-apple-darwin/release/bundle && hdiutil create -volname "AIjia" -srcfolder macos/AIjia.app -ov -format UDZO dmg/AIjia_X.Y.Z_x64.dmg`

### Step 4: 上传到阿里云 OSS

```bash
python3 code/scripts/upload-to-oss.py X.Y.Z
```

脚本自动从 macOS Keychain 读取 OSS 凭证（服务名 `aijia-oss`），无需每次输入 AK/SK。
...
凭证读取优先级：环境变量 `OSS_ACCESS_KEY_ID`/`OSS_ACCESS_KEY_SECRET` > macOS Keychain。

文件来源优先级：本地构建产物 > GitHub Release 下载。脚本自动探测本地 DMG，找不到时尝试从 GitHub 下载。

上传到 `aijia/vX.Y.Z/` 版本目录，并复制到 `aijia/latest/`（Landing Page 下载链接指向 latest）。
...
- 仓库是 private，`upload-to-oss.py` 本地文件找不到时尝试 GitHub 下载（可能因 private 仓库失败，需 `gh run download` 获取 CI artifacts）
...
| `code/scripts/upload-to-oss.py` | OSS 上传脚本（Keychain 凭证 + 本地优先） |
