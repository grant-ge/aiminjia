...
3. **并行 notarize（自 v0.5.25 起）**：DMG 和 .app 同时提交 Apple notary，每个独立 60min timeout，submission id + rc 写盘后台监控；任一失败保留 tmp dir 调试。串行版（v0.5.24）总耗时被 Apple 端排队叠加；并行版接近单次延迟。
